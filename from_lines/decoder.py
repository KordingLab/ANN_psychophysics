    ...
    def __init__(self, layer):
        ...
        if self.layer == 4:
            ...
            self.deconv = torch.nn.Sequential(
                ...
                torch.nn.Tanh(),
                torch.nn.Conv2d(in_channels=2, out_channels=2, kernel_size=5, stride=1, padding=2)
            )
        elif self.layer == 16:
            # starts [64, 256, 28, 28]
            self.deconv = torch.nn.Sequential(
                ...
                torch.nn.Tanh(),
                torch.nn.Conv2d(in_channels=2, out_channels=2, kernel_size=9, stride=1, padding=4)
            )
        elif self.layer == 23:
            # starts [64, 512, 14, 14]
            self.deconv = torch.nn.Sequential(
                ...
                torch.nn.Tanh(),
                torch.nn.Conv2d(in_channels=2, out_channels=2, kernel_size=17, stride=1, padding=8)
            )
        elif self.layer == 30:
            # starts [64, 512, 7, 7]
            self.deconv = torch.nn.Sequential(
                ...
                torch.nn.Tanh(),
                torch.nn.Conv2d(in_channels=2, out_channels=2, kernel_size=33, stride=1, padding=16)
            )
        else:
