from data_generators import (single_black_line, get_quadratures,
                             ...
                             add_random_polygon_perimeter_n, add_random_ellipse_perimeter
                             )
import pandas as pd
import numpy as np
...
def gen_batch_random_ellipse_perimeter(red, green, blue, num_lines_per_img):
    all_inputs = list()
    all_targets = list()
    n_samples = 10000
    for n in range(n_samples):
        image_array = np.zeros((3, 224, 224))
        input = add_random_ellipse_perimeter(image_array, red, green, blue, num_lines_per_img)
